 ...
 * - Project State tables (project_state, decision_log_entries, chat_messages)
 *
...
// Project State required tables
const PROJECT_STATE_TABLES = ['project_state', 'decision_log_entries', 'chat_messages'];
...
// SQL to create Project State tables (project_state, decision_log_entries, chat_messages)
const PROJECT_STATE_SQL = `
...
async function runMigrations() {
  ...
  try {
    ...
    if (authExisting.length > 0) {
      console.log(`✅ Found tables: ${authExisting.join(', ')}`);
    }

    // Check for missing Setup Wizard tables
    console.log('\n🔍 Checking for Setup Wizard tables...');
    ...
    if (wizardExisting.length > 0) {
      console.log(`✅ Found tables: ${wizardExisting.join(', ')}`);
    }

    // Check for missing Project State tables
    console.log('\n🔍 Checking for Project State tables...');
    const { existingTables: projectExisting, missingTables: projectMissing } =
      await checkMissingTables(pool, PROJECT_STATE_TABLES);

    if (projectExisting.length > 0) {
      console.log(`✅ Found tables: ${projectExisting.join(', ')}`);
    }

    // Check if any migrations are needed
    const totalMissing = [...authMissing, ...wizardMissing, ...projectMissing];
    ...
    console.log(`\n⚠️  Missing tables: ${totalMissing.join(', ')}`);
    console.log('\n🚀 Running migrations...');
    ...
    if (authMissing.length > 0) {
      ...
      if (authStillMissing.length > 0) {
        console.error(`❌ Failed to create Better-Auth tables: ${authStillMissing.join(', ')}`);
        process.exit(1);
      }
      console.log(`   ✅ Created: ${authMissing.join(', ')}`);
    }
    ...
    if (wizardMissing.length > 0) {
      ...
      if (wizardStillMissing.length > 0) {
        console.error(`❌ Failed to create Setup Wizard tables: ${wizardStillMissing.join(', ')}`);
        process.exit(1);
      }
      console.log(`   ✅ Created: ${wizardMissing.join(', ')}`);
    }

    // Run Project State migrations if needed
    if (projectMissing.length > 0) {
      console.log('   Creating Project State tables...');
      await pool.query(PROJECT_STATE_SQL);

      // Verify Project State tables were created
      const { missingTables: projectStillMissing } =
        await checkMissingTables(pool, PROJECT_STATE_TABLES);

      if (projectStillMissing.length > 0) {
        console.error(`❌ Failed to create Project State tables: ${projectStillMissing.join(', ')}`);
        process.exit(1);
      }
      console.log(`   ✅ Created: ${projectMissing.join(', ')}`);
    }
